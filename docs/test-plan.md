...
| C.10 | Egress-Whitelist blockiert             | `monoceros run -- bash -c 'if timeout 3 bash -c "echo > /dev/tcp/example.com/443"; then echo ALLOWED; else echo BLOCKED; fi'`                                            | Output: `BLOCKED`. Beweist: iptables-Rules aus dem Runtime-Image greifen, `example.com` ist nicht auf der Default-Allowlist. Bei `ALLOWED`: `cap_add: [NET_ADMIN]` fehlt im `compose.yaml` — Entrypoint loggt Warnung und Egress läuft offen.                                                                                                                                                                                                                                                                                        | Task 8    |
| C.11 | Egress-Whitelist lässt Erlaubtes durch | `monoceros run -- bash -c 'if timeout 3 bash -c "echo > /dev/tcp/api.anthropic.com/443"; then echo ALLOWED; else echo BLOCKED; fi'`                                      | Output: `ALLOWED`. Gegenprobe zu C.10 — die Default-Allowlist enthält `api.anthropic.com`, also kommt die Connection durch. Bei `BLOCKED` ohne C.10-Fail: DNS funktioniert nicht (UDP/TCP 53 sollte unconditional offen sein) oder Hostname-Resolution beim Container-Start ist gescheitert.                                                                                                                                                                                                                                         | Task 8    |
