...
| C.10 | `monoceros apply` rebuildet nach `add-*` (compose) | Vorbedingung: laufende Compose-Solution (aus C.1 — Solution-Name `demo`, ergibt Compose-Projekt `demo_devcontainer` und Volume `demo_devcontainer_postgres-data`. Für eine anders benannte Solution den Namen entsprechend ersetzen: das Pattern ist `<solution-folder-basename>_devcontainer_<volume-name>`). Host-seitig _vorher_ den Volume-Stempel notieren: `docker volume inspect <solution>_devcontainer_postgres-data --format '{{.CreatedAt}}'`. Dann: `monoceros add-language python --yes && monoceros apply`. Anschließend `monoceros run -- python3 --version`, `monoceros status` und host-seitig `docker volume inspect <solution>_devcontainer_postgres-data --format '{{.CreatedAt}}'` erneut. | `apply`-Output zeigt zuerst `Stopping containers (volumes preserved)…` + docker-compose-down-Output, dann `Bringing devcontainer up at …` + `devcontainer up`-Output mit Feature-Build-Layer für Python. Nach `apply`: `python3 --version` antwortet mit `Python 3.x.y` (Feature ist im neuen Container — vor `apply` war Python nicht da). `monoceros status` zeigt `workspace` und `postgres` beide `running`. Volume-`CreatedAt`-Stempel ist vor und nach dem `apply` **identisch** → der `down`-Schritt hat das Volume nicht angefasst, Postgres-Daten überleben den Rebuild. Image-Mode-Variante (separate bare Solution): `monoceros apply` ruft `devcontainer up --workspace-folder <root> --remove-existing-container`, alter Workspace-Container ist weg, neuer hat das hinzugefügte Feature. | M2.5 Task 2 |
...
- C.10 fehlerhaft, mehrere Symptome:
  - `python3 --version` nicht gefunden nach `apply` → Feature wurde
    ...
    der Rebuild den Layer übersprungen — `monoceros down && monoceros start` manuell probieren.
  - Volume-`CreatedAt` ändert sich nach dem `apply` → der `down`-Schritt
    hat doch Volumes mitgerissen. `runDown` setzt `volumes: false`; falls
    der Stempel trotzdem neu ist, prüf ob die compose-Down-Args
    versehentlich `-v` enthalten (host-seitig: `docker compose -f .devcontainer/compose.yaml -p demo_devcontainer down --dry-run` zeigt die Args).
  - `docker volume inspect` schlägt fehl mit „No such volume" → entweder
    der Volume-Name weicht ab (Compose-Projekt-Name = Solution-Folder-
    Name + `_devcontainer`) oder Postgres ist nie hochgekommen. Diagnose:
...
| E.1.7 | Workspace-Layout (`projects/` + `.code-workspace`) | `ls -la` im Solution-Root                                                                                          | Sichtbar: `projects/` (mit `.gitkeep` drin, sonst leer), `<solution>.code-workspace`. `cat <solution>.code-workspace` zeigt `{ "folders": [{ "path": "." }] }` (Single-Root am Anfang — `monoceros add-repo` ergänzt später Subfolder).                                                                                                                                                                                                        | M2.5 Task 1           |
| E.1.8 | `.code-workspace` lädt VS Code als Multi-Root      | Host-seitig in VS Code: `File → Open Workspace from File…` → das `<solution>.code-workspace` auswählen             | VS Code-Statuszeile zeigt „Workspace: \<solution>", Explorer listet `.` als einen Root mit den System-Dotfoldern. Beim Klick auf „Reopen in Container" startet der Devcontainer wie bisher; die Workspace-Definition bleibt nach Container-Start sichtbar.                                                                                                                                                                                     | M2.5 Task 1           |
| E.1.9 | `findSolutionRoot()` walked aus `projects/<name>/` | im Container: `mkdir -p projects/dummy && cd projects/dummy && monoceros status` (oder `monoceros run -- echo ok`) | Command findet die Solution-Root via Aufwärtswalk und führt aus. Bedingung dafür, dass `/monoceros:iterate` aus einem geklonten Repo-Subfolder funktioniert. Wenn das schiefgeht: `findSolutionRoot` in [`packages/cli/src/devcontainer/locate.ts`](../packages/cli/src/devcontainer/locate.ts) bricht zu früh ab — sehr unwahrscheinlich, weil generisch geschrieben, aber wenn doch: ein Test in `shell.test.ts` deckt genau diesen Pfad ab. | M2.5 Task 1           |
...
- E.1.7 fehlerhaft → entweder `projects/`-mkdir oder das Schreiben
  der `.code-workspace`-Datei fehlt in
  [`packages/cli/src/create/index.ts`](../packages/cli/src/create/index.ts);
  oder die Datei wurde mit altem Inhalt regeneriert (ältere Stage-E-Solution
  vor M2.5 Task 1 — frische Solution anlegen)
- E.1.8 fehlerhaft → VS Code lädt das Workspace-File nicht. Häufige
  Ursache: User hat den Folder geöffnet (`File → Open Folder`), nicht
  das Workspace-File (`File → Open Workspace from File`). Beide funktionieren,
  aber nur letzteres aktiviert die Multi-Root-Ansicht.
- E.1.9 fehlerhaft → `findSolutionRoot` zerbricht am Subfolder. Diagnose:
  `pnpm --filter @monoceros/cli run test -- shell.test.ts` host-seitig
  ausführen; der Test „resolves from a projects/<name>/ subfolder to the
  solution root" muss grün sein.
