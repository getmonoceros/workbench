...
| C.10 | `monoceros apply` rebuildet nach `add-*` (compose) | Vorbedingung: laufende Compose-Solution mit Postgres (aus C.1). Schreibe ein paar Test-Daten rein: `monoceros run -- bash -c 'PGPASSWORD=postgres psql -h postgres -U postgres -c "CREATE TABLE t(x int); INSERT INTO t VALUES(42);"'`. Dann: `monoceros add-language python --yes && monoceros apply`. Anschließend `monoceros run -- python3 --version` und `monoceros run -- bash -c 'PGPASSWORD=postgres psql -h postgres -U postgres -tc "SELECT x FROM t;"'` | `apply`-Output zeigt zuerst `Stopping containers (volumes preserved)…` + docker-compose-down-Output, dann `Bringing devcontainer up at …` + `devcontainer up`-Output mit Feature-Build-Layer für Python. `python3 --version` antwortet danach mit Python-Version (Feature ist im neuen Container). `psql`-Query gibt `42` zurück → Postgres-Volume hat den Rebuild überlebt. Image-Mode-Variante (separate bare Solution): `monoceros apply` ruft `devcontainer up --workspace-folder <root> --remove-existing-container`, alter Workspace-Container ist weg, neuer hat das hinzugefügte Feature. | M2.5 Task 2 |
...
- C.10 fehlerhaft, mehrere Symptome:
  ...
  - Postgres-Query gibt nichts/Fehler zurück → der `down`-Schritt hat
    Volumes mitgerissen. `runDown` schreibt `volumes: false`; falls doch
    weg, prüf ob das Volume `<solution>_devcontainer_postgres-data`
    noch existiert: `docker volume ls | grep postgres-data`.
