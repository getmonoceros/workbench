...
import { refreshLogInContainer } from './refresh-log.js';
...
export function buildPostCreateScript(opts: CreateOptions): string {
  const lines: string[] = [
    ...
    '',
    '# Per-feature refresh hooks (ADR 0054). A feature whose tool talks to a',
    '# cloud service and releases constantly drops a script into',
    '# /usr/local/share/monoceros/refresh.d/ during its install.sh. The tool',
    '# version lives in a cached image layer, and this container was just',
    '# recreated from that layer, so without this pass the builder starts on',
    '# whatever was current when the layer was first built — and loses any',
    '# self-update the previous container had applied. Each hook compares the',
    '# installed version against the published one and installs only on a',
    '# difference, so the common case costs one version check.',
    '#',
    '# Runs BEFORE the login hooks below: a login belongs to the tool version',
    '# the builder is about to use, not to the one being replaced.',
    '#',
    '# Best-effort by construction. No network, a registry outage or a broken',
    '# upstream release must never fail an apply — the tool from the image is',
    '# still there and still works. A failing hook warns and the pass',
    '# continues with the next one.',
    `REFRESH_LOG="${refreshLogInContainer(opts.name)}"`,
    'mkdir -p "$(dirname "$REFRESH_LOG")"',
    'rm -f "$REFRESH_LOG"',
    'if [ -d /usr/local/share/monoceros/refresh.d ]; then',
    '  for hook in /usr/local/share/monoceros/refresh.d/*.sh; do',
    '    [ -f "$hook" ] || continue',
    '    echo "→ refresh hook: $(basename "$hook")"',
    '    if ! MONOCEROS_REFRESH_LOG="$REFRESH_LOG" bash "$hook"; then',
    '      echo "  ! refresh hook failed: $(basename "$hook")" >&2',
    "      printf '%s: could not check for updates\\n' \\",
    '        "$(basename "$hook" .sh)" >>"$REFRESH_LOG" 2>/dev/null || true',
    '    fi',
    '  done',
    'fi',
    '',
    ...
    '# run in lexicographic order, each in its own subshell.',
    '#',
    '# A failing hook warns and the pass continues (ADR 0054). It used to',
    '# abort the whole of post-create under `set -e`, which meant one',
    "# feature's expired credential took down every later hook, the feature",
    '# notes and the workspace dependency install with it.',
    ...
    '    bash "$hook" || echo "  ! post-create hook failed: $(basename "$hook")" >&2',
    '  done',
    'fi',
    '',
  ...
  // Anything that clones over https inside the container needs the helper: a
  // declared repo, or an agent plugin marketplace. Gating it on repos alone
  // left a plugin-only workbench with a credentials file nothing read, and a
  // clone that died asking for a password.
  const clonesOverHttps =
    (opts.repos ?? []).some((r) => r.url.startsWith('https://')) ||
    (opts.pluginMarketplaceUrls ?? []).length > 0;
  if (clonesOverHttps) {
    lines.push(
      '',
