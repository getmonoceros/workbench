...
EOF
  ...
  # acli publishes only a `latest` download and exposes no version endpoint,
  # so there is nothing to compare against before fetching — the only way to
  # learn the current version is to download it. That makes the per-apply
  # check of the other features the wrong shape here: it would pull the
  # binary on every single apply for an answer that is usually "no change".
  #
  # So this one is rate-limited to once a day via a stamp in the workspace
  # (which survives the container being recreated). That still fixes both
  # cases that matter: a workbench set up weeks after the layer was built
  # gets a current acli, and a re-apply never silently moves backwards.
  REFRESH_DIR=/usr/local/share/monoceros/refresh.d
  mkdir -p "${REFRESH_DIR}"
  cat >"${REFRESH_DIR}/atlassian-rovodev.sh" <<'HOOK'
#!/usr/bin/env bash
# Auto-generated by the Monoceros atlassian feature (rovodev/acli).
#
# Keeps `acli` current across applies (ADR 0054). See the install-time comment
# for why this is rate-limited rather than version-checked: acli ships a
# `latest` URL and no version endpoint.
#
...
version_of() {
  acli --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -n1
}

installed="$(version_of)"

# The stamp lives next to the refresh log, i.e. in the bind-mounted workspace,
# so it outlives the container. Without the log path there is no stable place
# to put it, and a hook that cannot rate-limit itself should not download.
if [ -z "${MONOCEROS_REFRESH_LOG:-}" ]; then
  exit 0
fi
STAMP="$(dirname "${MONOCEROS_REFRESH_LOG}")/refresh-stamps/acli"
...
if [ -f "${STAMP}" ]; then
  ...
  if [ "$((now - last))" -lt "${MAX_AGE}" ]; then
    note "acli ${installed:-unknown} (checked within the last day)"
    exit 0
  fi
fi
...
case "${ARCH}" in
  amd64 | arm64) ;;
  *)
    note "acli: unsupported architecture ${ARCH:-unknown}, keeping ${installed:-the version from the image}"
    exit 0
    ;;
esac
...
if ! curl -fsSL --max-time 120 -o "${TMP}" \
  "https://acli.atlassian.com/linux/latest/acli_linux_${ARCH}/acli" 2>/dev/null; then
  note "acli: download failed, keeping ${installed:-the version from the image}"
  rm -f "${TMP}"
  exit 0
fi

if sudo install -o root -g root -m 0755 "${TMP}" /usr/local/bin/acli 2>/dev/null; then
  printf '%s\n' "${now}" >"${STAMP}" 2>/dev/null || true
  after="$(version_of)"
  if [ "${after}" = "${installed}" ]; then
    note "acli ${after:-unknown} (already current)"
  else
    note "acli ${after:-unknown} (updated from ${installed:-unknown})"
  fi
else
  note "acli: install failed, keeping ${installed:-the version from the image}"
fi
rm -f "${TMP}"
HOOK
  chmod 0755 "${REFRESH_DIR}/atlassian-rovodev.sh"
fi
...
EOF
  ...
  # ─── Refresh hook (ADR 0054) ────────────────────────────────────
  # Same shape as the acli hook and for the same reason: twg installs through
  # an official install script that always fetches the current build, with no
  # version endpoint to ask first. Rate-limited to once a day via a stamp in
  # the workspace rather than run on every apply.
  #
  # Ordered to run after the tool update but before the skills hook in
  # post-create.d: `twg skills install` should register the skills of the twg
  # the builder is about to use.
  REFRESH_DIR=/usr/local/share/monoceros/refresh.d
  mkdir -p "${REFRESH_DIR}"
  cat >"${REFRESH_DIR}/atlassian-twg.sh" <<'HOOK'
#!/usr/bin/env bash
# Auto-generated by the Monoceros atlassian feature (twg).
#
# Keeps `twg` current across applies (ADR 0054). twg releases constantly, has
# no self-update and prints no update notice, so without this the builder
# stays on the build-time version for the life of the image layer.
#
# Rate-limited to once a day: the official install script fetches the current
# build unconditionally, so there is no cheap way to ask "is there anything
# new" first.
#
# Deliberately no `set -e`: best-effort by contract. twg's installer has
# changed its flags before, and a failure here must never fail an apply.
set -u
...
version_of() {
  twg --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -n1
}
...
STAMP="$(dirname "${MONOCEROS_REFRESH_LOG}")/refresh-stamps/twg"
mkdir -p "$(dirname "${STAMP}")" 2>/dev/null
...
if [ -f "${STAMP}" ]; then
  ...
  if [ "$((now - last))" -lt "${MAX_AGE}" ]; then
    note "twg ${installed:-unknown} (checked within the last day)"
    exit 0
  fi
fi
...
if ! curl -fsSL --max-time 60 -o "${TMP}" \
  https://teamwork-graph.atlassian.com/cli/install 2>/dev/null; then
  note "twg: could not fetch the installer, keeping ${installed:-the version from the image}"
  rm -f "${TMP}"
  exit 0
fi

# Same flags as the feature's build-time install: pin the install dir so both
# root and node find it on PATH, and stay fully unattended. `--yes` is only
# honoured together with --skip-login AND --skip-skills; the skills are
# (re-)installed by the post-create hook as the node user.
if sudo bash "${TMP}" \
  --install-dir /usr/local/bin \
  --skip-login \
  --skip-skills \
  --yes >/dev/null 2>&1; then
  # The installer's mktemp + chmod leaves the binaries at 0700, which the node
  # user cannot execute. Same fix as at build time.
  for f in /usr/local/bin/twg-bin /usr/local/bin/twg; do
    [ -f "$f" ] && sudo chmod 0755 "$f" 2>/dev/null
  done
  printf '%s\n' "${now}" >"${STAMP}" 2>/dev/null || true
  after="$(version_of)"
  if [ "${after}" = "${installed}" ]; then
    note "twg ${after:-unknown} (already current)"
  else
    note "twg ${after:-unknown} (updated from ${installed:-unknown})"
  fi
else
  note "twg: installer failed, keeping ${installed:-the version from the image}"
fi
rm -f "${TMP}"
HOOK
  chmod 0755 "${REFRESH_DIR}/atlassian-twg.sh"
...
if [ "${FORGE}" = "true" ]; then
  ...
  # Forge gets NO refresh hook, unlike the other tools in this feature
  # (ADR 0054). Not an omission: the version Forge should run is decided by
  # this container's Node, not by what npm published last. The engine ranges
  # below mean "newest compatible", and that answer only changes when the base
  # image changes — which is `monoceros upgrade`'s business, not an apply's. A
  # refresh hook here would either reinstall the same version every day or
  # fight the exclusion its author declared on purpose.
  #
